...
SolverMMTZ::SolverMMTZ(IloEnv & env, Data & data): 
		cplex(env), modelo(env), x(env), y(env), u(env), datos(data), ambiente(env){
}

void SolverMMTZ::solve_model(std::string file_name, IloInt multiplicador, IloInt Max_Z1, IloInt Max_Z2)
{
	...
	for (int i = 0; i < datos.n; i ++)
	{
		for (int j = 0; j < datos.n; j++)
		{
			if (i != j && i != datos.deposito){
				modelo.add(u[i] >= u[j] + 3*y[i][j]*datos.t[i][j] + x[i][j]*datos.t[i][j] - M*(1 - x[i][j] - y[i][j]));}
		}
	}
...
void SolverMMTZ::graphSolution(std::string file_name)
{
	if(solver_success)
	{
...
void SolverMMTZ::saveResult(std::string file_name, IloInt multiplicador)
{
