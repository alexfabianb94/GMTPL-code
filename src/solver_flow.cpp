...
SolverFlow::SolverFlow(IloEnv & env, Data & data): 
		cplex(env), modelo(env), x(env), y (env), f(env), datos(data), ambiente(env){
}

void SolverFlow::solve_model(std::string file_name, IloInt multiplicador, IloInt Max_Z1, IloInt Max_Z2)
{
    ...
    f = IloArray <IloIntVarArray> (ambiente, datos.n);
	for (int i = 0; i < datos.n; i++)
	{
		x[i] = IloBoolVarArray(ambiente, datos.n);
		y[i] = IloBoolVarArray(ambiente, datos.n);
		f[i] = IloIntVarArray(ambiente, datos.n, 0, datos.n);
	}
	...
	for (int j = 0; j < datos.n; j++){
		...
		if (j != datos.deposito){
			...
			modelo.add(suma_izq + 1 == suma_der);
			suma_izq.end();
			suma_der.end();
	...
	for (int p = 0; p < datos.numero_clusters; p++)
	{
		for (std::set<IloInt>::iterator i = datos.clusters[p].begin(); i != datos.clusters[p].end(); i++){
			for (int j = 0; j < datos.n; j++)
			{
				if (datos.clusters[p].find(j) == datos.clusters[p].end() && *i != j){
					modelo.add(f[*i][j] <= datos.n*x[*i][j]);
				}
			}
		}
	}
	...
	//ambiente.out()  << "Restriccion 9 ingresada correctamente" << endl;
...
void SolverFlow::showSolution()
{
	if(solver_success)
	{
		...
		for (int i = 0; i < datos.n; i++)
		{
			...
			{
				if (i != j){
					if (cplex.getValue(f[i][j]) >= 0.98)
					{
						ambiente.out() << "\nf[" << i + 1 << "][" << j + 1 << "] = " << cplex.getValue(f[i][j]);
						Valor_Z2 += datos.t[i][j] * cplex.getValue(f[i][j]);
					}
				}
			}
		}
...
void SolverFlow::graphSolution(std::string file_name)
{
	if(solver_success)
	{
...
void SolverFlow::saveResult(std::string file_name, IloInt multiplicador)
{
