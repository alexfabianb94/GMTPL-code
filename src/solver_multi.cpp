...
SolverMulti::SolverMulti(IloEnv & env, Data & data): 
		cplex(env), modelo(env), x(env), y (env), f(env), datos(data), ambiente(env){
}

void SolverMulti::solve_model(std::string file_name, IloInt multiplicador, IloInt Max_Z1, IloInt Max_Z2)
{
...
void SolverMulti::graphSolution(std::string file_name)
{
	if(solver_success)
	{
...
void SolverMulti::saveResult(std::string file_name, IloInt multiplicador)
{
